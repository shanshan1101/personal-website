...
app.post('/api/contact', async (req, res) => {
  try {
    ...
    // 单独处理确认邮件发送，即使失败也不影响主邮件的成功响应
    try {
      await transporter.sendMail(confirmationMailOptions);
      console.log('确认邮件发送成功');
    } catch (confirmationError) {
      console.error('确认邮件发送失败，但主邮件已成功发送:', confirmationError);
    }

    // 无论确认邮件是否成功，只要主邮件成功就返回成功响应
    console.log('准备返回成功响应给前端');
    res.json({ success: true, message: '消息发送成功！' });
    console.log('成功响应已返回');
  } catch (error) {
    console.error('主邮件发送失败:', error);
    console.error('错误详情:', error.stack);
    console.log('准备返回错误响应给前端');
