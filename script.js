...
async function sendAIMessage() {
    ...
    // 直接调用智谱AI HTTP API
    try {
        // 从环境变量获取API Key和模型配置（在实际项目中应从服务器获取或使用安全的方式存储）
        const API_KEY = '8cf433d4562d4103a3e0ce317e3974ed.vr7TY8s0TdF4Xd5r';
        const MODEL = 'glm-4.6';
        const API_URL = 'https://open.bigmodel.cn/api/paas/v4/chat/completions';
        
        console.log('正在调用智谱AI HTTP API...');
        ...
        if (res.ok && data && data.choices && data.choices[0] && data.choices[0].message) {
            ...
            // API返回错误
            console.warn('智谱AI API返回错误:', data?.error || '未知错误');
